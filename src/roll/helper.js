...
export function roll(minimum, maximum) {
  return Math.floor(Math.random() * (maximum - (minimum + 1))) + minimum;
}

export function d20Roll(modifier = 0) {
  const r = roll(1, 20);
...
export function damageRoll(weapon) {
  const minMax = weapon.minMax;

  return roll(minMax[0], minMax[1]);
}

export function getAdvantageOrDisadvantage(entity, ability) {
...
export function getArmorClass(entity, armorMap) {
...
export function makeSavingThrow(rollFunc = d20Roll, entity, ability, bonusOrPenalty) {
  return rollFunc(getModifier(entity.getAbility(ability)) + entity.getProficiencyBonus() + bonusOrPenalty)[1];
}

export function applyAdvantageOrDisadvantage(roll1, roll2, advantageOrDisadvantage = 'advantage') {
